...
import random
import pygame
from sprites import BunnySprite, ArrowSprite ,BadguySprite 

def initGame():
    pygame.init()
    pygame.mixer.init()
    screen = pygame.display.set_mode(congif.screenSize)
    pygame.display.set_caption("Bunny Hitman")
    game_images = {}
    for key,value in congif.Images.items():
        game_images[key] = pygame.image.load(value)

    game_sounds = {}
    for key, value in congif.Sounds.items():
        game_sounds[key] = pygame.mixer.Sound(value)
    return screen , game_sounds, game_images

def main():
    screen, game_images, game_sounds =  initGame()
    pygame.mixer.music.load(congif.Sounds['moonlight'])
    pygame.mixer.music.play(-1, 0.0)
    font = pygame.font.Font(None, 24)
    bunny = BunnySprite(image = game_images.get('rabbit'), position = (100, 100))
    acc_record = [0. , 0.]
    healthval = 194
    arrow_sprites_groups = pygame.sprite.Group()
    Badguy_sprites_groups = pygame.sprite.Group()

    badguy = BadguySprite(image = game_images.get('badguy'). position(640 , 100))
    Badguy_sprites_groups.add(badguy)

    badguy_timer  = 100
    badguy_timer = 0
    
    running , exit = True , False
    clock = pygame.time.Clock()

    while running:
        scrren.fill(0)
        for x in range(congif.screenSize[0]//game_images['grass'].get_width()+1):
                 for y in range(congif.screenSize[1]//game_images['grass'].get_width()+1):
        ...
        for event in pygame.event.get():
            if event.type() == pygame.QUIT:
                ...
                arrow_sprites_groups.add(arrow)
            

        key_pressed = pygame.key.get_pressed()
        if key_pressed[pygame.K_w] :
            bunny.move(congif.screenSize,'up' )
        elif key_pressed[pygame.K_s] :
            bunny.move(congif.screenSize,'down' )
        elif key_pressed[pygame.K_a] :
            bunny.move(congif.screenSize,'left' )
        elif key_pressed[pygame.K_d] :
            bunny.move(congif.screenSize,'right' )
